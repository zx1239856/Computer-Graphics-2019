...
namespace utils {
    ...
    __device__ inline bool isPointLeftOfSplittingPlane(const utils::Vector3 &p) const {
        switch (split_axis) {
            case X_AXIS:
    ...
    __device__ inline uint32_t getNeighboringNode(utils::Vector3 &p) const {
...
struct KDTree_GPU {
    ...
    KernelArray<uint32_t> tri_indices;
    uint32_t root;

private:
    __device__ inline bool singleRayStacklessIntersectImpl(uint32_t node, const Ray &r, double &t_entry, double &t_exit,
        ...
        while (t_entry < t_exit && t_entry > t_entry_prev) {
            ...
            while (!nodes._array[node].isLeaf())
                node = nodes._array[node].isPointLeftOfSplittingPlane(hit) ? nodes._array[node].left : nodes._array[node].right;
            for (uint32_t i = nodes._array[node].start_tri_idx; i < nodes._array[node].start_tri_idx + nodes._array[node].num_tris; ++i) {
                const TriangleFace &tri = faces._array[tri_indices._array[i]];
                auto v1 = vertices._array[tri.v1];
                auto v2 = vertices._array[tri.v2];
                auto v3 = vertices._array[tri.v3];
                auto n1 = norms._array[tri.n1];
                auto n2 = norms._array[tri.n2];
                auto n3 = norms._array[tri.n3];
            ...
            auto aabb_intersect = intersectAABBInOut(r, nodes._array[node].p0, nodes._array[node].p1);
            ...
            node = nodes._array[node].getNeighboringNode(hit_exit);
...
public:
    __device__ pair<double, utils::Vector3> singleRayStacklessIntersect(const Ray &r) const {
        utils::Vector3 norm;
        auto res = intersectAABBInOut(r, nodes._array[root].p0, nodes._array[root].p1);
        double t_near = res.second, t_far = res.third;
        if (res.first) {
            bool hit = singleRayStacklessIntersectImpl(root, r, t_near, t_far, norm);
            if (hit)
                return {t_far, norm};
            else
    ...
    __device__ pair<utils::Vector3, utils::Vector3> getAABB() const {
            return {nodes._array[root].p0 - EPSILON, nodes._array[root].p1 + EPSILON};
    }
    ...
    public:
        ...
        KDTree_GPU gpu_tree =  {makeKernelArr(vertices), makeKernelArr(faces), makeKernelArr(norms), makeKernelArr(cpu_nodes), makeKernelArr(tri_indices), root};
        return gpu_tree;
    }
