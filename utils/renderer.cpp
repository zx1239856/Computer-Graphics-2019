...
utils::Vector3 basic_pt(const Scene &scene, const Ray &ray, int depth, unsigned short *X) {
    ...
    auto && f = texture.pt.getColor(std::get<2>(intersect), X);
    double p = f.first.max();
    if (p < EPSILON)
        return texture.pt.emission;
    if (++depth > 5) {
        if (erand48(X) < p)f.first = f.first * (1 / p);
        else return texture.pt.emission;
    }
    if (f.second == DIFF) {
        ...
        return texture.pt.emission + f.first.mult(basic_pt(scene, Ray(x, d), depth, X));
    } else if (f.second == SPEC)
        return texture.pt.emission + f.first.mult(basic_pt(scene, Ray(x, ray.direction.reflect(nl)), depth, X));
    else {
        Ray reflray = Ray(x, ray.direction.reflect(nl));
        Vector3 d = ray.direction.refract(n, into ? 1 : texture.pt.re_idx, into ? texture.pt.re_idx : 1);
        if (d.len2() < EPSILON) // total internal reflection
            return texture.pt.emission + f.first.mult(basic_pt(scene, reflray, depth, X));
        double a = texture.pt.re_idx - 1, b = texture.pt.re_idx + 1, R0 = a * a / (b * b), c =
                1 - (into ? -ray.direction.dot(nl) : d.dot(n));
        double Re = R0 + (1 - R0) * c * c * c * c * c, Tr = 1 - Re, P = .25 + .5 * Re, RP = Re / P, TP = Tr / (1 - P);
        return texture.pt.emission + f.first.mult(depth > 2 ? (erand48(X) < P ?
