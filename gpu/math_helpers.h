...
inline int toUInt8(double x) { return int(pow(clamp(x), 1 / 2.2) * 255 + .5); }

__device__ inline utils::Vector3 uniformSampleOnHemisphere(double u1, double u2) {
    double sin_theta = sqrt(fmax(.0, 1.0 - u1 * u1));
    double phi = 2. * M_PI * u2;
    return utils::Vector3(cos(phi) * sin_theta, sin(phi) * sin_theta, u1);
}

__device__ inline utils::Vector3 cosWeightedSampleOnHemisphere(double u1, double u2) {
